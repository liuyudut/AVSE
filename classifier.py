...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, data_loader, _train_X, _train_Y,  _nclass, _input_dim, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, pretrain_classifer=''):
        ...
        if pretrain_classifer == '':
            self.fit()
            #self.acc = self.val(data_loader.test_seen_feature, data_loader.test_seen_label, data_loader.seenclasses)
    ...
    def val(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            with torch.no_grad():
                if self.cuda:
                    output = self.model(test_X[start:end].cuda())  ## , volatile=True??
                else:
                    output = self.model(test_X[start:end])  ## , volatile=True??
